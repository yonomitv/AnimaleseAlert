...
# for shorter time constraints, daanzu works better than en-us-0.22
model = Model(config.VOSK_MODEL)
device = config.MIC_ID
recognizer = KaldiRecognizer(model, 16000)
audio_queue = queue.Queue()

ENABLE_OFFLINE_TESTING = config.ENABLE_OFFLINE_TESTING

file_id = 0

ANIMALESE_DURATION = config.ANIMALESE_DURATION
...
# Callback function to be called by sounddevice for each audio block
def callback(indata, frames, time, status):
    if status:
        print(status, flush=True)
    # Convert the audio data to 16-bit PCM format and put it in the queue
    audio_queue.put(bytes(indata))

def SpeechToText():
    # clear the completed audio folder and reset file id
    CompletedAudioFolder = Path() / "CompletedAudio"
    NewAudioFolder = Path() / "NewAudio"
    for entry in CompletedAudioFolder.iterdir():
        print(entry)
        os.remove(entry)
    file_id = 0
    
    for entry in NewAudioFolder.iterdir():
        print(entry)
        os.remove(entry)

    print("CompletedAudio Folder Cleared")
    # we signal to alert app to start and fade in the image
    # alert app will move it over to completed audio which would get deleted before each run above
    start_file_path = NewAudioFolder / "start.txt"
    with start_file_path.open(mode="w") as start_file:
        pass
    exitloop = True
    while exitloop:
        for entry in NewAudioFolder.iterdir():
            # alert app will fade in and create this file when done
            # let's us know when to start. 
            # so we also handle it's deletion.
            if entry.is_file() and entry.stem == "fadeindone":
                exitloop = False
    ...
    # Set the audio stream with desired sample rate and channels
    with sd.RawInputStream(samplerate=16000, channels=1, callback=callback, blocksize=32000, dtype="int16", device=device):
        start_time = time.time()
        print("Listening...")
        while True:
            time_elapsed = time.time() - start_time
            if (time_elapsed > ANIMALESE_DURATION):
                # Nothing needs to be written here, its existence is enough
                output_file = Path() / "NewAudio" / f"finished.txt"
            ...
            if recognizer.AcceptWaveform(data):
                ...
                if result_dict["text"]:
                    ...
                    print()
                    GenerateAnimaleseForAlertApp(result_dict["text"], file_id)
                    file_id += 1
...
# we hang here until we get an alert
# for dev testing
def TestWaitForRedeem():
    print("Waiting for animalese redeem...")
    while True:
        time.sleep(CHECK_TWITCH_INTERVAL)
        print("Animalese redeem came through")
        return

def CheckForFadeOutCompleted():
    NewAudioFolder = Path() / "NewAudio"
    exitloop = True
    while exitloop:
        for entry in NewAudioFolder.iterdir():
            # alert app will fade out and create fadeoutdone.txt when finished
            # let's us know when the alert is done and off the screen. 
            # so we also handle it's deletion.
            if entry.is_file() and entry.stem == "fadeoutdone":
                exitloop = False
                entry.unlink()
                break
        time.sleep(1)
...
def main():
    if ENABLE_OFFLINE_TESTING:
        print("Running offline. Simulating app")
        while True:
            TestWaitForRedeem()
            SpeechToText()
            CheckForFadeOutCompleted()
        return
    if len(sys.argv) > 1:
        sentence = sys.argv[1]
