...
std::vector<std::filesystem::path> AudioQueue;
...
bool ProcessTranscriptData(void* appstate)
{
    ...
    if (state->TranscriptIndex >= 0 && state->TranscriptIndex < TranscriptData.size() - 1)
    {
        // tricky
...
/*
* There are 3 files that python will create when it is finished creating the alert
...
*    Will be named 1_F.txt. _F indicating finished. If the file exists, then animalese.py is done
*/
void CheckForNewAudio(void* appstate)
{
    ...
    for (const auto & entry : std::filesystem::directory_iterator("NewAudio"))
    {
        ...
        if (std::filesystem::is_regular_file(entry.path(), ec) && FileStem.ends_with("_F"))
        {
        #ifdef DEBUG_ENABLED
            std::cout << entry.path() << " " << entry.path().filename() << " " << Extension 
                      << " " << entry.path().stem() << " " << entry.path().stem().string().length() 
                      << std::endl;
        #endif

            // Might have scenarios where newer alerts play before older alerts
            // Would have to be pretty active redeeming
            // sorting is annoying in c++ and I don't wanna code a comparator 
            for (size_t i = 0; i < AudioQueue.size(); ++i)
            {
                if (AudioQueue[i] == FileStem)
                {
                    // file already in array, break
                    break;
                }
                // at last iteration, meaning, not already inserted into queue
                else if (i == AudioQueue.size() - 1)
                {
        ...
        //else
        //{
        //    std:: cout << entry.path() << " " << entry.path().filename() << " Is not a file" << std::endl;
        //}
    }

    AppState* state = (AppState*) appstate;
    // check if we can play an alert.  
    // If we can, it loads the audio and loads the transcript file 
    // Then changes the appstate which AppIterate handles to play the alert
    if (state->AppStateEnum == EAppState::Idle && state->gMusic == NULL && AudioQueue.size() > 0)
    {
        ...
        std::cout << "We here before transcript reading \n";
    #endif
...
/*
* Simply moves the alert files into the finished folder
* Makes checking for new audio easier to deal with
*/
void CleanUpLastAlertFiles()
{
    ...
    // we have the 1_F.txt file stored, now we're stripping it to reconstruct the other stems
    // some examples: 1_F -> 1, 15_F -> 15
    for (size_t i = 0; i < FileStem.length() - 2; ++i)
    {
        AudioFileStem += FileStem[i];
    }

    std::filesystem::rename(std::filesystem::current_path() / "NewAudio" / AudioFileStem += ".txt", std::filesystem::current_path() / "CompletedAudio" / AudioFileStem += ".txt");
    std::filesystem::rename(std::filesystem::current_path() / "NewAudio" / AudioQueue[0].stem() += ".txt", std::filesystem::current_path() / "CompletedAudio" / AudioQueue[0].stem() += ".txt");
    std::filesystem::rename(std::filesystem::current_path() / "NewAudio" / AudioFileStem += ".wav", std::filesystem::current_path() / "CompletedAudio" / AudioFileStem += ".wav");
...
SDL_AppResult SDL_AppIterate(void* appstate){
    ...
    {
	    if( state->gMusic && !MIX_TrackPlaying( state->gMusic ) )
	    {
		    //Play the music
        #ifdef DEBUG_ENABLED
    ...
    else if (state->AppStateEnum == EAppState::PlayingAlert)
    {
        ...
        state->AccumlatedTimeMS += state->deltaTime;
        //state->TargetAccumlatedTimeMS += state->deltaTime;

        // Floating point / double weirdness makes me unsure on this, but it works
        if (state->AccumlatedTimeMS >= state->TargetAccumlatedTimeMS)
        {
            ...
            // means we have finished
            //std::cout << " FINAL BRANCH: " << std::fabs(state->AccumlatedTimeMS - state->TargetAccumlatedTimeMS) << std::endl;
            if (!bSuccess)
            {
                state->AccumlatedTimeMS = 0;
                state->TargetAccumlatedTimeMS = 0;
    ...
    // Trying to minimze cpu usage when not running an alert
    // crashes on window drag unless you use borderless
    //else if (state->AppStateEnum == EAppState::DoNotPlayAlerts)
    //{
    //    if (!state->bRenderDirty)
    //    {
    //        SDL_RenderPresent(state->rendererData.renderer);
