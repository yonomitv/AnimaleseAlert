...
static void SDL_Clay_RenderFillRoundedRect(Clay_SDL3RendererData* rendererData, const SDL_FRect rect, const float cornerRadius, const Clay_Color _color) {
    ...
    vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {rect.x + clampedRadius, rect.y + clampedRadius}, color, {0, 0} }; //0 center TL
    vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {rect.x + rect.w - clampedRadius, rect.y + clampedRadius}, color, {1, 0} }; //1 center TR
    vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {rect.x + rect.w - clampedRadius, rect.y + rect.h - clampedRadius}, color, {1, 1} }; //2 center BR
    vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {rect.x + clampedRadius, rect.y + rect.h - clampedRadius}, color, {0, 1} }; //3 center BL
    ...
    for (int i = 0; i < numCircleSegments; i++) {
        ...
        for (int j = 0; j < 4; j++) {  // Iterate over four corners
            ...
            vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {cx + SDL_cosf(angle1) * clampedRadius * signX, cy + SDL_sinf(angle1) * clampedRadius * signY}, color, {0, 0} };
            vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {cx + SDL_cosf(angle2) * clampedRadius * signX, cy + SDL_sinf(angle2) * clampedRadius * signY}, color, {0, 0} };
    ...
    // Top edge
    vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {rect.x + clampedRadius, rect.y}, color, {0, 0} }; //TL
    vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {rect.x + rect.w - clampedRadius, rect.y}, color, {1, 0} }; //TR
    ...
    indices[indexCount++] = vertexCount - 1; //TR
    // Right edge
    vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {rect.x + rect.w, rect.y + clampedRadius}, color, {1, 0} }; //RT
    vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {rect.x + rect.w, rect.y + rect.h - clampedRadius}, color, {1, 1} }; //RB
    ...
    indices[indexCount++] = vertexCount - 1; //RB
    // Bottom edge
    vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {rect.x + rect.w - clampedRadius, rect.y + rect.h}, color, {1, 1} }; //BR
    vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {rect.x + clampedRadius, rect.y + rect.h}, color, {0, 1} }; //BL
    ...
    indices[indexCount++] = vertexCount - 1; //BL
    // Left edge
    vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {rect.x, rect.y + rect.h - clampedRadius}, color, {0, 1} }; //LB
    vertices[vertexCount++] = CLAY__INIT(SDL_Vertex){ {rect.x, rect.y + clampedRadius}, color, {0, 0} }; //LT
...
static void SDL_Clay_RenderArc(Clay_SDL3RendererData* rendererData, const SDL_FPoint center, const float radius, const float startAngle, const float endAngle, const float thickness, const Clay_Color color) {
    ...
    for (float t = thicknessStep; t < thickness - thicknessStep; t += thicknessStep) {
        ...
        if (rendererData->points.size() < numCircleSegments + 1)
        {
            rendererData->points.resize(numCircleSegments + 1);
        }
        for (int i = 0; i <= numCircleSegments; i++) {
            const float angle = radStart + i * angleStep;
            rendererData->points[i] = CLAY__INIT(SDL_FPoint){
                    SDL_roundf(center.x + SDL_cosf(angle) * clampedRadius),
                    SDL_roundf(center.y + SDL_sinf(angle) * clampedRadius) };
        }
        SDL_RenderLines(rendererData->renderer, rendererData->points.data(), numCircleSegments + 1);
    }
}
    ...
    for (size_t i = 0; i < rcommands->length; i++) {
        ...
        switch (rcmd->commandType) {
            ...
            case CLAY_RENDER_COMMAND_TYPE_TEXT: {
                ...
                TTF_SetTextColor(text, config->textColor.r, config->textColor.g, config->textColor.b, config->textColor.a * rendererData->alpha);
            ...
            case CLAY_RENDER_COMMAND_TYPE_BORDER: {
                ...
                if (config->cornerRadius.topLeft > 0) {
                    ...
                    SDL_Clay_RenderArc(rendererData, CLAY__INIT(SDL_FPoint){centerX, centerY}, clampedRadii.topLeft,
                ...
                if (config->cornerRadius.topRight > 0) {
                    ...
                    SDL_Clay_RenderArc(rendererData, CLAY__INIT(SDL_FPoint){centerX, centerY}, clampedRadii.topRight,
                        270.0f, 360.0f, config->width.top, config->color);
                }
                if (config->cornerRadius.bottomLeft > 0) {
                    const float centerX = rect.x + clampedRadii.bottomLeft -1;
                    const float centerY = rect.y + rect.h - clampedRadii.bottomLeft;
                    SDL_Clay_RenderArc(rendererData, CLAY__INIT(SDL_FPoint){centerX, centerY}, clampedRadii.bottomLeft,
                ...
                if (config->cornerRadius.bottomRight > 0) {
                    ...
                    SDL_Clay_RenderArc(rendererData, CLAY__INIT(SDL_FPoint){centerX, centerY}, clampedRadii.bottomRight,
            ...
            case CLAY_RENDER_COMMAND_TYPE_SCISSOR_START: {
                ...
                currentClippingRectangle = CLAY__INIT(SDL_Rect) {
                        .x = (int)boundingBox.x,
                        .y = (int)boundingBox.y,
            ...
            case CLAY_RENDER_COMMAND_TYPE_IMAGE: {
                ...
                SDL_SetTextureAlphaModFloat(texture, rendererData->alpha);
